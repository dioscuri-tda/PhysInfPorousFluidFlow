# see: https://github.com/mkisantal/backboned-unet/blob/master/backboned_unet/unet.py
import torch
...
class Unet(nn.Module):
    ...
    def __init__(self,
        ...
        self.backbone, self.shortcut_features, self.bb_out_name = get_backbone(backbone_name, weigths=weigths)
        ...
        #self.final_conv = nn.Conv2d(decoder_filters[-1], classes, kernel_size=(3, 3), padding=1, padding_mode='circular')
        #self.final_conv = nn.Conv2d(decoder_filters[-1], classes, kernel_size=(1, 1))
        self.final_conv = nn.Conv2d(decoder_filters[-1], classes, kernel_size=(final_conv_kernel_size, final_conv_kernel_size), padding=(final_conv_kernel_size-1)//2,
                                    padding_mode='circular')
        if smooth_mode != 'none':
            oneD_kernel = cv2.getGaussianKernel(smooth_kernel_size, sigma=0)
            twoD_kernel = oneD_kernel * oneD_kernel.T
            twoD_kernel = twoD_kernel.astype(np.float32)
            W = torch.tensor(twoD_kernel)
            W = W.view(1,1,smooth_kernel_size, smooth_kernel_size).repeat(2, 1, 1, 1)
            self.smooth = nn.Conv2d(2, 2, padding=(smooth_kernel_size-1)//2, padding_mode='circular', kernel_size=smooth_kernel_size, bias=False, groups=2)
            if smooth_mode == 'gaussian_fixed':
                self.smooth.weight = nn.Parameter(torch.Tensor(W), requires_grad=False)
            if smooth_mode == 'gaussian':
                self.smooth.weight = nn.Parameter(torch.Tensor(W), requires_grad=True)
